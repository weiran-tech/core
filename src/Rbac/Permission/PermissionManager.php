...
class PermissionManager
{
    ...
    public function cachedPermissionKv(string $key = null)
    {
        static $permissions;
        if (!$permissions) {
            $permissions = sys_tag('wr-core')->remember(PyCoreDef::ckPermissionKv(), config('cache.ttl', 600), function () {
                $data = collect();
    ...
    {
        return sys_tag('wr-core')->remember(PyCoreDef::ckPermissionNames(), config('cache.ttl', 600), function () {
    ...
    {
        sys_tag('wr-core')->del(PyCoreDef::ckPermissionNames());
    }
}
